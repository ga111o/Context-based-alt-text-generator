...
def invoke_agent(language, title, context, image_path):
    translations = {
        "Korean": f"Image captioner가 생성한 설명을 이미지 주변의 텍스트 '{context}'와 연결지어 최종적으로 이미지에 대한 자연스러운 설명을 작성하세요. '{context}'와 관련된 부분을 찾기 위해 Object detector를 사용해도 됩니다. 최종 설명은 원래 이미지의 설명처럼 자연스럽고 일관성이 있어야 하며, 동시에 웹페이지의 제목 '{title}'에 기반해야 합니다. 당신의 최종 설명을 {language}로 번역하세요.",
                ...
                try:
                    ...
                    if db_result:
                        ...
                        if db_output:
                            ...
                            if DEBUG.PRINT_LOG_BOOLEN:
                                print(f" | {session} |---- already exist llm alt text {image_name}")
